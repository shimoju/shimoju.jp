...
ヘッダーの上paddingは`--space-section`と`--header-top-min`（6.4rem）の大きい方、下paddingは`--space-section`を全画面幅で使い、操作領域の寸法とは独立させる。フォーカス輪郭はoutline-offset: -2pxで内側に描く。配置比較UI・toggle-positionパラメーターの適用・JavaScriptによるDOM移動は廃止する。
...
大きな余白は本文基準の役割別トークンにする。本文画像・表・コードの前後は1.931em、カバーの上は本文サイズの1.931倍。記事一覧の区切り・ページ見出しの下・ヘッダー上下の基準は＋10段の2.560倍（4.352rem）、年別アーカイブ・脚注の上・ホーム紹介文の下・フッターの上は＋12段の3.089倍（5.2513rem）、記事末尾は＋13段の3.394倍（5.7698rem）、ページ送りの上は＋15段の4.096倍（6.9632rem）。rem換算は本文1.7remの場合で、CSSでは本文基準のcalcを維持する。ヘッダー上だけは配色ボタン用の下限6.4remを確保する。記事ヘッダー下はUI余白の3.2remとし、本文基準の区切り余白とは分ける。記事一覧は罫線の前後にgapとpaddingの両方があるため、合計の間隔で評価する。段落間隔は1em、引用の前後余白は1.6emとする。コード内部・インラインコードの余白、操作対象の最小寸法は各部品の仕様に従う。
