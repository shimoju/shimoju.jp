...
ヘッダーナビの横gapはデスクトップ2.4rem・モバイル1.6rem、タグ／カテゴリの横gapは1.6rem。ページ送りは上下padding 0.8remと中央揃えで通常時の高さを4.8remとし、拡大時は伸びる。
...
モックもHugo標準の`.Lastmod`を取得して年月日で比較する。部品ページの更新日はfront matterから取得する。本実装での表示条件は`and (not .Lastmod.IsZero) (ne (.Date.Format "2006-01-02") (.Lastmod.Format "2006-01-02"))`とする。公開日・更新日の横gapとタグの横gapは1.6rem、縦gapはそれぞれ0.4rem・0.8rem。
