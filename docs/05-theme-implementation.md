...
シェア・はてなスターはモックでも実サービスへ接続する。自作SVG＋通常リンクを採用し、はてなスターのみ公式スクリプトを読み込む。フッターのX・GitHub・RSSも同じSVGの仕組みを使う。Xへのタグ由来ハッシュタグは付けない。本実装では共有対象に記事・AboutのPermalinkを用い、location.hrefやモック専用パラメーターを使わない。[公式設定と実装上の注意](07-theme-sharing.md)を参照する。モックの固定年・記事件数・分類件数や`noindex,nofollow`を本番へそのままコピーしない。
...
モックもHugo標準の`.Lastmod`を取得して年月日で比較する。部品ページの更新日はfront matterから取得する。本実装での表示条件は`and (not .Lastmod.IsZero) (ne (.Date.Format "2006-01-02") (.Lastmod.Format "2006-01-02"))`とする。公開日・更新日の横gapとタグの横gapは1rem、縦gapはそれぞれ0.25rem・0.5rem。
