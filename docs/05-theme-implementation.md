...
共有・プロフィール・配色切り替えの図形は`mock/icons.mjs`へ集約し、`icon(name)`で共通の`viewBox="0 0 24 24"`・幅高さ24・`preserveAspectRatio="xMidYMid meet"`を出力する。ロゴは元の形を縦横同率で変換して中央配置した座標を保持し、図柄ごとの境界表や実行時の座標変換は持たない。月・太陽は線幅1.5を前提に外周がキャンバス内へ収まる座標とし、CSSの同じ線幅を使う。コピーも同じviewBoxを用い、配色・コピーは共通CSSで表示サイズ16px相当・操作領域32px相当・小角丸4pxを指定する。外部のはてなスターはこの正規化の対象外。
