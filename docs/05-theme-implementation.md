...
ヘッダーのpadding-blockは`var(--space-group) var(--space-section)`とし、全画面幅で上は本文の3.089倍・下は2.560倍にする。ナビの操作領域が作る下側の空きを考慮し、上側の余白を広く取る。フォーカス輪郭はoutline-offset: -2pxで内側に描く。配置比較UI・toggle-positionパラメーターの適用・JavaScriptによるDOM移動は廃止する。
...
大きな余白は本文基準の役割別トークンにする。本文画像・表・コードの前後は1.931em、カバーの上は本文サイズの1.931倍。記事一覧の区切り・ページ見出しの下・ヘッダーの下は＋10段の2.560倍（4.352rem）、年別アーカイブ・脚注の上・ホーム紹介文の下・ヘッダーとフッターの上は＋12段の3.089倍（5.2513rem）、記事末尾は＋13段の3.394倍（5.7698rem）、ページ送りの上は＋15段の4.096倍（6.9632rem）。rem換算は本文1.7remの場合で、CSSでは本文基準のcalcを維持する。記事ヘッダー下はUI余白の3.2remとし、本文基準の区切り余白とは分ける。記事一覧は罫線の前後にgapとpaddingの両方があるため、合計の間隔で評価する。段落間隔は1em、引用の前後余白は1.6emとする。コード内部・インラインコードの余白、操作対象の最小寸法は各部品の仕様に従う。

本文内のH2〜H6の基本余白は前2.121em・後0.687em。スケールの＋8段／−4段から算出して小数第3位に丸め、見出し自身の文字サイズに追従させる。直後のpはmargin-topを0にし、見出しの下marginで間隔を決める。連続する下位見出しの前は0.910em（−1段）とする。同階層・上位への遷移と独立ブロックの余白は維持する。記事タイトルは対象外とし、最初の本文要素は上余白0。連続見出しでは上下marginの大きい側が実間隔となる。行高・コードサイズ・操作対象の寸法にスケールを強制しない。

引用罫線は`--quote-border`で独立させ、ライトはLatte Overlay1 #8c8fa1、ダークはMocha Overlay1 #7f849cを使う。太さ2px、本文色・背景・一般境界線は維持する。

本文H2〜H6のIDを保持し、同じIDを指すネイティブリンクを追加する。見出しをposition: relative、リンクをabsolute・right: 100%・幅・最小高さ`--heading-anchor-size`（2.4rem）・高さ1.5emとする。justify-content: flex-endとし、spanをflex: 0 0 var(--ui-space-4)・text-align: centerで右寄せし、記号の配置位置を維持する。疑似要素は使わず、操作領域とフォーカス枠を一致させる。共通ガターは基本`--gutter: 16px`、`(min-width: 640px)`では24pxとし、その後に置く`(hover: hover)`で`max(24px, var(--heading-anchor-size))`へ上書きする。ホバー可能環境では既定文字サイズを小さくしても24px未満にしない。見出しへの左paddingは設けず、本文と左端を揃える。共通のshellは最大720pxなので、既に十分な左右余白がある広い画面では配置は変わらない。リンクはdisplay: noneを基本とし、`@media (hover: hover)`でdisplay: flexにする。ホバー可能環境でもopacity: 0を基本に、見出しの:hover／:focus-withinで表示する。画面幅やタッチ対応の有無だけでは判定しない。アンカーは本文付随のリンクなので48pxの独立操作部品ルールから除外し、フォーカス輪郭は内側に描く。表示制御にJavaScriptは使わない。
...
- 単位なし：行高。`--leading-body: 1.9`、`--leading-heading: 1.5`、`--leading-label: 1.5`、`--leading-table: 1.6`、`--leading-code: 1.3`。各役割は独立した値として定義する。適用範囲は[視覚要件の行高](04-theme-visual-requirements.md#行高)を参照。
- px：本文最大幅720px、左右ガターの基準24px／16px、ブレークポイント640px、細い罫線・角丸。ホバー可能環境のガターには見出しリンク操作幅2.4remの下限も設ける。

行高は役割の境界で指定する。一覧リンク全体に見出し用行高を指定せず、タイトル・日付・要約で分ける。前後記事タイトルはblock化して`--leading-heading`を適用し、親リンクの行ボックスによる行高の拡大を防ぐ。見出しアンカーの高さは`calc(var(--leading-heading) * 1em)`で見出しの行高に追従させる。
...
ヘッダーナビ・記事末尾タグ・タグ／カテゴリ一覧は、全画面幅で横gapに`--text-link-gap`（0.8rem）、リンクの左右paddingに`--text-link-inset`（0.4rem）を共用する。隣接する文字領域の間隔は左右paddingとgapを合わせて1.6remとなる。ナビは中央揃えのため光学的な左補正は行わない。分類名と件数の内部gapは0.4remとし、項目間より狭くする。ページ送りは上下padding 0.8remと中央揃えで通常時の高さを4.8remとし、拡大時は伸びる。

### 記事末尾・分類リンクの光学的な左揃え

`--term-optical-inset: min(var(--text-link-inset), var(--gutter))`を記事末尾タグと分類一覧で共有し、列のmargin-inline-startにその負値を使う。シェアは`--share-optical-inset: min(calc((var(--control-size) - var(--icon-size)) / 2), var(--gutter))`の負値をshare-mountに適用する。シェア内部だけでなく外側のflex itemを移動することで、はてなスターとのgapを維持する。タグ・分類のpaddingや操作高さ、シェアの48px四方、列内のgapは変更しない。折り返し後も各行の先頭に同じ補正が効く。フッターには適用しない。

フォーカス輪郭は`.article-tags a`・`.terms a`・`.share-icons a`のfocus-visibleだけoutline-offset: -2pxにし、左に寄せても欠けないようにする。ガター上限はタッチ端末の拡大方式を仮定せず、文字だけが拡大された場合にも操作領域を画面内に保つための安全策。SVGの図柄固有の左右余白はそのままにし、共通の24px表示領域を配置基準とする。
...
コピーはコード本文だけを対象とし、行番号の要素を除外して空白・改行を維持する。SVGは通常・チェック・×の3状態を用意し、`data-copy-state`で切り替える。Clipboard APIのPromise成功後にのみチェックとCopiedを通知し、3秒後に通常アイコン・Copy codeへ戻す。失敗時は×とCopy failed・再試行・手動選択をtitleとアクセシブルな名前・通知で案内する。ボタンのSVGをtextContentで置換せず、再試行可能にする。読み上げ通知は`role="status"`で提供し、本文下に可視メッセージを増やしてレイアウトを動かさない。

コードブロックはposition: relativeとし、Copyを右上にabsolute配置する。常設ツールバーを置かず、ラベルがある場合だけ高さを使う。実際のボタンは32px相当、疑似要素の背景面は内側4pxの24px相当、SVGは16px相当とし、操作領域を疑似要素で外へ拡張しない。ボタンは右上8px相当の位置に置き、背景面の外周をブロック端から12px相当に揃える。ラベルの右余白は操作領域とは独立した48px相当とする。1行のコードも行高18.2px＋上下余白32px＋境界線2pxで約52.2pxとなり、操作領域が収まるためブロックの最小高さは指定しない。背景はBase、アイコンはmuted、通常の境界線と小角丸を使う。コード本文の行高・ハイライトは維持する。

余白は既存の8px刻みに基づき、外周16px・ラベルとコードの間8pxとする。外周より内部の距離を小さくして、ラベルとコードを一つの内容としてまとめる。preのpaddingは全方向`--code-inset`とし、ラベルがある場合だけpreの上paddingを`--ui-space-2`にする。ラベル自身の上・左は`--code-inset`を共用する。コードブロックは縦方向のflexでラベルとコードを積む。highlightにはmin-width: 0を指定し、長いコードがブロックを押し広げずpre内でスクロールするようにする。余白はグリフの見た目ではなく行ボックス間で評価し、フォント固有の上下空きとは区別する。

初期HTMLのCopyにはhiddenを付け、JSでイベント設定時に解除する。通常はopacity: 0・pointer-events: noneとし、hover可能環境のブロック:hover、:focus-within、タップによるdata-copy-visibleで表示する。タップは標準のclickイベントを使い、preventDefaultやコード全体のボタン化は行わない。Escape・外側pointerdown・preのscrollでdata-copy-dismissedを設定し、hover／focus-within中でも隠せる。Copyにフォーカスがある場合は、隠す前にpreへ移す。マウス再進入・フォーカス移動・再タップで再表示でき、横スクロール・長押し・選択は標準動作を維持する。[ホバー・フォーカスで表示する内容の扱い](https://www.w3.org/WAI/WCAG22/Understanding/content-on-hover-or-focus.html)

## リンクと配色の実装境界

小アイコンの操作領域は32px四方・角丸4pxとし、24px四方を内包する形にする。既定文字サイズ16pxでWCAG 2.2 AAの最小ターゲットサイズ24pxを満たす設計とし、AAAの44pxを小アイコンの要件とはしない。[最小ターゲットサイズ](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html)

共有・プロフィール・配色切り替えの図形は`mock/icons.mjs`へ集約し、`icon(name)`で共通の`viewBox="0 0 24 24"`・幅高さ24・`preserveAspectRatio="xMidYMid meet"`を出力する。サービスの特徴を線画に簡略化した自作アイコンを使い、図柄ごとの境界表や実行時の座標変換は持たない。コピーも同じviewBoxを用いる。共通CSSで`fill: none`・`stroke: currentColor`・丸い線端と角を指定し、共有・プロフィールは24px表示で線幅1.5、配色・コピーは16px表示で線幅2.25とする。表示上の線幅はともに1.5px相当となり、文字拡大にも追従する。共有・プロフィールの図形は、曲線の実際の極値を含む中心線の長辺を22.5に合わせ、両側の線幅0.75を加えて24に収める。図形の拡大率は縦横共通とし、描画範囲の中心を(12, 12)に揃える。線幅自体は拡大しない。操作領域は共有・プロフィール48px相当、配色・コピー32px相当で、小角丸4pxとする。外部のはてなスターは対象外。
...
ルートに`scrollbar-gutter: stable`を指定し、通常型スクロールバーの有無によるページ間の横移動を抑える。Copyボタンとコード本文は、それぞれ`.copy:focus-visible`と`.code-block pre:focus-visible`の`outline-offset: -2px`で共通の2px輪郭を内側に描き、親の角丸とoverflowによる切れを防ぐ。Copyの操作領域はブロック内に収める。単独の本文動画はブロック表示と`--space-block`の前後余白を使い、figure内では動画自身のmarginを0にして二重化を避ける。Archivesの日付は`--text-meta`を使う。
...
下線色も同じ描画元へ`text-decoration-color: var(--muted)`を明示し、リンクの`:is(:hover, :focus-visible)`では自身と対象の子要素だけ`var(--text)`にする。文字色は変更しない。通常は下線なしのテキストナビも、ホバー・キーボードフォーカス時に同じ範囲へ下線を表示する。本文・脚注・現在地は常時下線を維持し、サイト名・見出しリンク・アイコンは常に下線なしとする。

一覧は一記事を一つのネイティブリンクで囲み、`aria-labelledby`で記事タイトルをリンク名にする。長い要約全体の読み上げや重複したTab停止を避け、新しいタブで開く操作を保つ。別ボタンを追加するときはリンクの入れ子を作らない。Archivesもタイトルと日付を同じリンクにする。リンクは縦方向flex・gapに`--title-meta-gap`を使い、日付のmarginは0とする。タイトルを`.archive-title`で囲んで`--leading-heading`を適用し、月名・日付には`--leading-label`を使う。ホバー下線はタイトルだけに付ける。月のgridは`grid-template-columns: var(--ui-space-8) minmax(0, 1fr)`で月名列を3.2remに固定し、記事列に残りの幅を割り当てる。列間gapは1.6remを基本とし、640px以上で4.8remへ切り替える。`align-items: first baseline`、月見出しはmargin 0とし、先頭記事のベースラインに合わせる。月名と記事タイトルの文字サイズ・行ボックス高が異なっていても、先頭行のベースラインを基準に整列する。記事リストは縦方向flexと`--ui-space-6`（24px）のgapを使い、月末の記事にも余白が付くliのmarginは設けない。月間隔は`--ui-space-8`（32px）、最後の月はmargin-bottom 0。年の間隔は`--space-group`を維持する。記事詳細・About・一覧・Archivesのタイトルと日付の間隔は`--title-meta-gap`（0.8rem）を共用する。分類一覧は名称を`.term-name`で囲み、ホバー下線はこの要素だけに付ける。件数も含むリンク全体のクリック範囲とフォーカスを維持する。前後記事は`rel="prev"`／`rel="next"`で役割を明示し、全画面幅で左列／右列の1行目へ配置する。列は`repeat(2, minmax(0, 1fr))`で同幅とし、列間は全画面幅で16px相当。DOM上の順番や隣接要素の有無を役割判定に使わない。ページ送りと前後記事の方向ラベルは共通の`.nav-label`で生成し、`display: inline-block; white-space: nowrap`で記号と文字の半角スペースを保持する。Prev／Nextにpostは付けない。
