...
コードブロックはposition: relativeとし、Copyを右上にabsolute配置する。常設ツールバーを置かず、ラベルがある場合だけ高さを使う。実際のボタンは40px相当、疑似要素の背景面は内側8pxの24px相当、SVGは16px相当とし、操作領域を疑似要素で外へ拡張しない。ボタンは右上4px相当の位置に置く。1行のコードも行高18.2px＋上下余白32px＋境界線2pxで約52.2pxとなり、操作領域が収まるためブロックの最小高さは指定しない。背景はBase、アイコンはmuted、通常の境界線と小角丸を使う。コード本文の行高・ハイライトは維持する。
