...
共有・フッターのSVGは24×24のviewBoxと`currentColor`を共用し、ライトではLatte Text、ダークではMocha Textに追従する。アイコンは2.4752rem、リンクは両方とも幅・高さ4.352remの正方形。flex-shrink: 0で横方向だけの縮小を防ぎ、フッター側で高さを上書きしない。ボタン間隔は1.2818rem、スターとの間隔は2.0519remとし、狭い幅では折り返す。
