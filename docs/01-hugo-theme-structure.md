...
前提とバージョンは[調査概要](README.md)を参照。ここでは「Hugoが描画するために必要なもの」と「このブログのモックに必要なもの」を区別する。

以下は調査時の候補一覧・実装例。今回のモックで採用する画面、表示項目、画面幅は[資料4の合意済み視覚要件](04-theme-visual-requirements.md)を参照する。
...
このサイトで現在公開している導線を維持する前提の優先順位。検索やProfileモードまで最初から実装する必要はない。

| 優先度 | 画面・URL例 | HugoのKind / 主なテンプレート候補 | モックに含める内容 |
| --- | --- | --- | --- |
| 必須 | ホーム `/` | `home` / `home.html` → `list.html` | サイト紹介、SNSリンク、記事一覧、ページ送り。現サイトはHome-Info型 |
| 必須 | 記事詳細 `/2026/09/01/development-environment-2026/` | `page` / `posts/page.html`、`page.html`、`single.html` | タイトル、日付、著者、本文、コード、画像、タグ、前後記事、共有 |
| 必須 | 固定ページ `/about/` | `page` / `page.html`、`single.html`、独自`layout` | プロフィール、長文、リンク。記事メタ情報や共有の表示有無を決める |
...
| 必須 | 個別タグ・カテゴリ `/tags/hugo/`、`/categories/技術/` | `term` / `term.html` → `list.html` | 分類見出し、その分類の記事、RSS導線、ページ送り |
| 必須 | 年月別アーカイブ `/archives/` | `page` + `layout: archives` / `archives.html` | 年月・記事数・記事へのリンク。Hugo固有のKindではない |
| 必須 | 404 | `404` / `404.html` | 見つからないことの説明、ホーム等への復帰導線 |
| 状態として必須 | 一覧2ページ目以降 `/page/2/` 等 | 元の一覧と同じテンプレート | 初ページのみの紹介を消す、前後リンクの端の状態 |
| 採用時 | 検索 `/search/` | `page` + `layout: search` / `search.html` | 入力、取得中、結果あり・0件・取得失敗、キーボード操作 |
| 採用時 | Profile型ホーム | `home`の表示バリエーション | アバター・紹介・ボタン中心。通常の記事一覧との導線 |
| 採用時 | 多言語ページ | 同じKind、言語別テンプレートも可 | 言語切り替え、翻訳なし、RTL、長い翻訳文 |
...
| 対象 | 確認する状態 |
| --- | --- |
| 共通枠 | モバイル320〜390px・タブレット・デスクトップ、light/dark、メニューが横幅を超える場合、長いサイト名、キーボードフォーカス |
...
| メタ情報 | 複数タグ、長いタグ、日付なし、更新日、複数著者、下書きプレビュー、共有ボタンの折り返し |
| 目次 | 見出しなし、深い階層、日本語アンカー、開閉、長い見出し、同名見出し |
| アクセシビリティ | 200%拡大、Tab操作、フォーカス表示、reduced motion、JS無効でも記事を読める、画像が読み込めない |

上記は自作テーマの検証用提案。すべてがHugoの自動機能、またはPaperModで品質保証済みという意味ではない。
...
| 検索用`index.json` | テーマ側でスキーマとテンプレートを作る | 検索対象フィールド、除外、件数・サイズ、言語、公開してよい情報 |
...
| `llms.txt`等 | 採用時に出力形式とテンプレートを用意 | 対象と用途。SEOの必須ファイルとして扱わない |
...
この4ファイルはHugo v0.165.0の最小サイトでビルド確認済み。home、ページ送り、section、記事、日付のない固定ページ、taxonomy、term、空sectionを確認した。実用テーマとして必要なデザイン・SEO等の完成品ではない。
...
1. 必須画面をモバイルとデスクトップで作り、記事本文の部品一覧を用意する。dark modeを採用するなら両配色を確認する。
...
6. [資料3の性能比較](03-papermod-analysis.md#自作テーマと比較するベンチマーク設計)でPaperModと比較する。
