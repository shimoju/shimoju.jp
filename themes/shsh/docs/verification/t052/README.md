...
その後、ユーザーが上記のMac Safari・固定previewの確認依頼を引用し「映像が見え、再生・一時停止できる」と回答した。[原文](../t055/safari-report.json)・V099により、動画の映像視認待ちは解消。T052の文字拡大・キーボードと合わせ、このタスクの範囲は確認済み。画面取得の空白の原因は断定しない。
...
Safariの実フォント・外部埋め込み、Chrome/Firefoxの残る項目は未確認のまま。T052の確認範囲は文字拡大・キーボードと、T055で補った動画表示・再生/一時停止に限定する。
