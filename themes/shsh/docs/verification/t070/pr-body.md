...
全59記事・About・Archivesと派生一覧の移行照合、代表画面・全画面レビューは実施済みです。承認済みの5代表環境の実機確認を完了し、最新headの必須CIと切り戻し先の照合後に本番移行します。
...
- テーマ実装`268bbb0`と、検証記録を含む`9f17369`の[GitHub統合検査](https://github.com/shimoju/shimoju.jp/actions/runs/35351594030)は246件成功。以後の差分は要件の承認記録・検証資料・証拠のみ。現在のPR headの成否はChecksで別途確認する。
...
- 実機完了判定はF026で承認されたMac Chrome・Mac Safari・Windows Chrome・iPhone Safari・Android Chromeの5代表環境。5代表環境の承認済み必須範囲は確認済みで、Safariのローカル動画とYouTube映像もユーザーが視認。ページ・条件の範囲と未確認は[実機記録表](https://github.com/shimoju/shimoju.jp/blob/hugo-theme/themes/shsh/docs/verification/real-devices.md)に明記。
- 確認用固定preview：[6f52990f](https://6f52990f.shimoju.pages.dev/)（268bbb0）。実機報告の対象版は各記録に保持する。

## マージ前後の残件

- マージ前：現在のPR headで必須CIと旧productionの切り戻し先を照合する。5代表環境の[証拠対応と制約](https://github.com/shimoju/shimoju.jp/blob/hugo-theme/themes/shsh/docs/verification/t070/README.md)を記録済み。
...
既知の制約として、公式Catppuccinコード色のコントラスト差と、Windows Firefoxの見出し500が通常フェイスになる差を承認済みです。F026によりWindows Edge/Firefox・Mac Firefox・iPadの未確認項目とMac Safari実フォント未取得は、必須対象外の制約として保持し、合格にはしていません。F027ではAndroidの実フォント未取得と文字のみ拡大未実施を制約として保持し、拡大確認にページズーム150%の結果を採用しました。F028でiPhone実フォント未取得も制約として保持することを承認済みです。未取得を合格にはせず、その他の条件を維持します。
