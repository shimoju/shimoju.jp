...
- 実機完了判定はF026で承認されたMac Chrome・Mac Safari・Windows Chrome・iPhone Safari・Android Chromeの5代表環境。Mac Chrome/SafariとAndroidの承認済み必須範囲は確認済みで、Safariのローカル動画とYouTube映像もユーザーが視認。ページ・条件の範囲と未確認は[実機記録表](https://github.com/shimoju/shimoju.jp/blob/hugo-theme/themes/shsh/docs/verification/real-devices.md)に明記。
- 確認用固定preview：[6f52990f](https://6f52990f.shimoju.pages.dev/)（268bbb0）。実機報告の対象版は各記録に保持する。

## マージ前後の残件

- マージ前：Windows Chrome・iPhone Safariの版、残る操作・埋め込み・JS無効、モバイルの実フォント・文字拡大など、[T064の環境別残件](https://github.com/shimoju/shimoju.jp/blob/hugo-theme/themes/shsh/docs/verification/t064/README.md)を確認する。確認済み項目は各環境で異なる。現在のPR headで必須CIを確認する。
...
既知の制約として、公式Catppuccinコード色のコントラスト差と、Windows Firefoxの見出し500が通常フェイスになる差を承認済みです。F026によりWindows Edge/Firefox・Mac Firefox・iPadの未確認項目とMac Safari実フォント未取得は、必須対象外の制約として保持し、合格にはしていません。F027ではAndroidの実フォント未取得と文字のみ拡大未実施を制約として保持し、拡大確認にページズーム150%の結果を採用しました。iPhoneなど他環境の免除は含みません。
