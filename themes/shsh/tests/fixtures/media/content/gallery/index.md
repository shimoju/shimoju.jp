---
title: 画像と動画
date: 2026-09-01
cover:
  image: screenshot.png
  alt: カバーのターミナル画面
---

![ターミナルの文字](screenshot.png)

{{< figure src="screenshot.png" alt="同じターミナルの文字" caption="画面の説明。**captionはaltと別**。" >}}

![縦長の文字資料](tall.png)

![実写真](photo.jpg)

![小さい透明画像](small.png)

![Assets画像](images/shared.png)

![Static画像](/images/static.png)

{{< figure src="diagram.svg" alt="SVG図" >}}

![静止GIF](still.gif)

![アニメーションGIF](animated.gif)

![アニメーションPNG](animated.png)

![アニメーションWebP](animated.webp)

![StaticアニメーションWebP](/images/animated.webp)

![StaticアニメーションPNG](/images/animated.png)

![StaticアニメーションGIF](/images/animated.gif)

{{< figure src="https://external.invalid/image.png" alt="外部画像" width="640" height="320" >}}

{{< video src="demo.mp4" width="1440" height="1076" title="ターミナルの操作" >}}

{{< speakerdeck id="457f092496ab4856b7c3cef5bcd2babb" ratio="1.77777777777778" >}}

前![文中の画像](small.png)後。

[![リンク内の画像](small.png)](https://example.org/)
