...
// Keep historical review submissions intact.
const target = ".cache/review-report";
mkdirSync(target, { recursive: true });
mkdirSync(`${target}/images`, { recursive: true });
for (const file of readdirSync(".cache/review-images").filter((file) => file.endsWith(".png")))
  cpSync(`.cache/review-images/${file}`, `${target}/images/${file}`);
const sections = [
  ["home", "ホーム"],
  ["posts", "記事一覧"],
  ["article", "実記事"],
      ...
      `<section id="${name}"><h2>${label}</h2>${[1440, 390]
        .flatMap((width) =>
          ["light", "dark"].map((color) => {
            ...
            return `<details${name === "specimen" && width === 390 && color === "light" ? " open" : ""}><summary>${condition}</summary><div class="pair">${[
              "shsh",
              "mock",
            ]
              .map((side) => {
                const title = side === "shsh" ? "shsh実装" : "モック＋承認差分F023";
                const file = `images/${name}-${width}-${color}-${side === "mock" ? "mock-approved-f023" : side}.png`;
...
writeFileSync(
  `${target}/index.html`,
  `<!doctype html>
<html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>承認差分を含む代表画面比較</title><style>
...
.note { max-width: 70em; }
</style></head><body><header><h1>承認差分を含む代表画面比較</h1>
<p class="note">ホーム・一覧・実記事・本文部品の比較です。左がshsh、右が承認差分F023（コード行高1.4）を適用したモック。「原寸を開く」リンクから画像を開けます。凍結モックと過去のレビュー提出物は変更しません。</p>
<nav aria-label="比較対象">${sections.map(([name, label]) => `<a href="#${name}">${label}</a>`).join("")}</nav>
...
<p class="note">全画面の比較はfull-reviewのレポートを参照してください。本文部品ではモック専用の文字・余白診断を除外しています。比較画像の外部ウィジェットは未読込です。実機・性能・配信検証と本番移行は未完了です。</p>
