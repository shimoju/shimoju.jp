...
const target = "docs/verification/r003";
mkdirSync(target, { recursive: true });
mkdirSync(`${target}/images`, { recursive: true });
for (const file of readdirSync(".cache/review-images").filter((file) =>
  file.startsWith("specimen-"),
))
          ...
          ["light", "dark"].map((color) => {
            ...
            return `<details${name === "specimen" && width === 390 && color === "light" ? " open" : ""}><summary>${condition}</summary><div class="pair">${[
              ...
              .map((side) => {
                ...
                const file = `${name === "specimen" ? "images" : "../r001/images"}/${name}-${width}-${color}-${side}.png`;
...
writeFileSync(
  `${target}/index.html`,
  `<!doctype html>
<html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>R003 指摘修正レビュー</title><style>
...
.note { max-width: 70em; }
</style></head><body><header><h1>R003 強調入力と開発環境の修正</h1>
<p class="note">ホーム・一覧・実記事・本文部品の比較です。左がshsh、右が凍結モック。「原寸を開く」リンクから画像を開けます。本文部品の強調を修正し、開発環境をthemes/shshへ集約しました。他3画面の画像は変更のないR001提出物を参照します。</p>
<nav aria-label="比較対象">${sections.map(([name, label]) => `<a href="#${name}">${label}</a>`).join("")}<a href="../r003-review.md">構造・差分・検証範囲</a></nav>
