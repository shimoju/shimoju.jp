...
assert.ok(about.includes("| 業務経験 | | |"), "F014 proposal must start from the unchanged source");
writeFileSync(
  `${source}/content/about.md`,
  about.replace("| 業務経験 | | |", "| 業務経験 | 技術 | 経験年数 |"),
);
execFileSync(
  "hugo",
  [
...
assert.equal(
  ...
  about,
  "Proposal may not mutate site content",
);
...
const tables = [1440, 390]
  .flatMap((width) =>
    ["light", "dark"].map(
      (color) =>
        `<details><summary>${width}px・${color}</summary><div class="pair">${["before", "proposal"].map((side) => `<figure><figcaption>${side === "before" ? "現状" : "修正案（未反映）"}</figcaption><a href="images/about-${width}-${color}-${side}.png"><img src="images/about-${width}-${color}-${side}.png" alt="About表 ${width}px ${color} ${side}" loading="lazy"></a></figure>`).join("")}</div></details>`,
    ),
  )
  .join("");
writeFileSync(
