...
ルートは全幅で106.25%、本文1rem・記事タイトル1.6rem・サイト名1.931remです。既定文字サイズ16pxでは本文のCSSサイズは17px。Safariの和文の見かけ上の縮小を考慮し、モバイルにも同じ割合を使います。コードブロックは`calc(14 / 17 * 1rem)`（標準設定で14px）・行高1.3。インラインコードは0.85emを維持します。ブラウザの既定文字サイズ変更にすべて追従します。
...
大きな余白は役割別トークンにする。本文画像・表・コードの前後は1.931em、カバーの上は1.931rem。記事一覧の区切り・ページ見出しの下は2.560rem、年別アーカイブ・脚注の上は3.089rem、記事末尾・デスクトップヘッダーの下は3.394rem、デスクトップヘッダーの上・ページ送りの上は4.096rem。モバイルヘッダーは上下2.560rem。記事一覧は罫線の前後にgapとpaddingの両方があるため、合計の間隔で評価する。段落間隔は1em、引用の前後余白は1.6emとする。コード内部・インラインコードの余白、操作対象の最小寸法は各部品の仕様に従う。
