...
配色ボタンはヘッダー右上に独立して配置します。サイト名とAbout／Archives／Categories／Tagsはそれぞれ中央揃えとし、配色ボタンはナビに含めません。SVG16px・操作領域48px相当を維持し、ヘッダー上部に操作領域と上下8px相当の余裕を確保します。操作領域の右端を本文領域の右端に揃え、SVGの光学的な右補正は行いません。フォーカス輪郭は内側に描きます。配置比較UIと旧toggle-positionパラメーターによる切り替えは廃止しました。
