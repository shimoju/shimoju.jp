...
- 記事本文は実記事を使い、X埋め込みだけリンクへ置き換えています。記事・分類件数とページ数は収録分です。ホーム紹介文は現行Hugo設定から取得し、Aboutは現行本文のみを使います。部品ページの日付・lastmodは検証用です。Hugo標準の日付補完を維持し、dateとlastmodの年月日が異なる場合だけ更新日を表示します。
- 入口と診断サンプルはテーマ本来のUIではありません。読者向けUIは現在の英語ラベル・日本語説明の使い分けを正式採用します。
