...
import { engagement, icon } from './sharing.mjs';
...
const navLabel = direction => `<span class="nav-label">${direction === 'prev' ? '<span aria-hidden="true">«</span> Previous' : 'Next <span aria-hidden="true">»</span>'}</span>`;
...
function end(post) {
  ...
  return `<footer class="article-end"><div class="article-tags" aria-label="Tags">${tags}</div>${engagement(post)}<nav class="post-nav" id="post-navigation" aria-label="Adjacent posts">${older ? `<a rel="prev" href="${older.id}.html"><small lang="en">${navLabel('prev')}</small><span class="post-nav-title">${escape(older.title)}</span></a>` : ''}${newer ? `<a rel="next" href="${newer.id}.html"><small lang="en">${navLabel('next')}</small><span class="post-nav-title">${escape(newer.title)}</span></a>` : ''}</nav></footer>`;
}
